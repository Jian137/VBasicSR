    ...
    def forward(self, lqs):
        ...
        for iter_ in [1, 2]:
            for direction in ['backward', 'forward']:
                ...
                feats = self.propagate(feats, flows, module) # TODO
                if self.cpu_cache:
                    del flows
                    torch.cuda.empty_cache()
...
class nonlocal_attention(nn.Module):
    ...
    def forward(self, x):
        b, t, c, h, w = x.size() # 输入7帧
        q = x[:, 3, :, :, :] # 设定第4帧为当前帧

        weight=torch.exp(-0.5*(self.D/(self.std*self.std)))
        weight=weight.unsqueeze(0).repeat(b,1,1) # 高斯加权

        reshaped_x = x.view(b*t , c, h, w).contiguous() #F_{t+i}
        h_ = self.norm(reshaped_x) #F_{t+i}
        q_=self.norm(q)  #F_{t}

        g_x = self.g(h_).view(b, t, self.inter_channels, h,w).contiguous() # 生成V
        theta_x = self.theta(h_).view(b, t, self.inter_channels,  h,w).contiguous() #生成K
        ...
        for i in range(t): # 遍历帧
            ...
            if self.is_training:
                f = torch.matmul(phi_x, theta) # Q.*K
            else:
                f = torch.matmul(phi_x.half(), theta.half())

            f_div_C = F.softmax(f, dim=-1)*weight # 高斯加权
            if self.is_training:
                y = torch.matmul(f_div_C, g).float() #V.* 高斯加权
            else:
        ...
        corr_prob = torch.cat(corr_l, dim=1).view(b, -1, h, w) # X_t
        W_y = self.W_z(corr_prob) #X_t 后1*1卷积
...
class CFNA(nn.Module):
    # CrossFrame Non-Local Attention
    def __init__(self, config, is_training=True):
        super(CFNA, self).__init__()
        self.in_channels = config['in_channels']
        self.inter_channels = self.in_channels // 2
        self.is_training=is_training
        width=config['width']
        ...
        if self.is_training:
            ...
            D=D.half()
        self.D=torch.nn.Parameter(D,requires_grad=False)  # type: ignore
        self.std=torch.nn.Parameter(4*torch.ones(1).float())
        if self.is_training==False:
            self.std=self.std.half()
        self.W_z1 = nn.Conv2d(in_channels=self.inter_channels, out_channels=self.in_channels, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # q 为 F_t W_y1 为Y_t?
        # return z, qloss
        return W_y,phi_x_for_quant
class MAA(nn.Module):
    # Memory_augmented Attention
    pass
