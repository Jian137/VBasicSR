...
# imdnv2 改 添加bank模块
# imdnv3 加上了mma模块
from .wlbank2 import Decoder,VectorQuantizer,Encoder
...
class IMDNV3(nn.Module):
    def __init__(self, in_nc=3, nf=64, num_modules=6, out_nc=3, upscale=4):
        super(IMDNV3, self).__init__()

        self.fea_conv = conv_layer(in_nc, nf, kernel_size=3)

        self.encoder=Encoder(nf,2)
        self.vec = VectorQuantizer(nf,nf)
        self.decoder = Decoder(nf,nf,'gn','leakyrelu')
        self.mma = MMA(nf)
        # IMDBs
    ...
    def forward(self, input):
        ...
        mer_out = self.encoder(out_fea)
        mma_out = self.mma(mer_out)
        mer_out = self.vec(mer_out)

        vec_loss = mer_out[1]
        mer_out = self.decoder(mer_out[0]+mma_out)
        ...
        out_lr = self.LR_conv(out_B) + out_fea+mer_out
        output = self.upsampler(out_lr)
        return output,vec_loss
