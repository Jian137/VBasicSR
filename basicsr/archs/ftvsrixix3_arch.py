...
class FTVSRIXIx3(nn.Module):
    ...
    def __init__(self, mid_channels=64, num_blocks=60, stride=4, keyframe_stride=3,spynet_pretrained=None,
        ...
        self.stride = stride
        # self.out_ch = mid_channels # TODO 偏移计算
        self.in_ch = 64
        self.out_ch = 64
        self.kernel_size = 3
        self.df_stride = 1
        ...
        self.dfnet = DFNet(pretrained=spynet_pretrained,ou_ch=self.out_ch,offset_groups=self.offset_groups,kernel_size=self.kernel_size)# TODO 切换为DFNet
        self.feat_extractor = ResidualBlocksWithInputConv(
            1, mid_channels, 5)# TODO 特征提取部分，修改输入通道设置
        ...
        self.upsample2 = PixelShufflePack(mid_channels, 64, 3, upsample_kernel=3)
        ...
        self.FTT = FTT(dct_kernel=dct_kernel, d_model=d_model, n_heads=n_heads,flow_channel=2*self.offset_groups * self.kernel_size * self.kernel_size)

        self.offset_warp = offset_warp(self.in_ch,self.out_ch,self.groups,self.kernel_size)
        self.location_warp = location_warp()
        #### 可变形卷积参数
    ...
    def compute_offset(self,lrs):
        n, t, c, h, w = lrs.size()
        lrs_1 = lrs[:, :-1, :, :, :].reshape(-1, c, h, w)
        lrs_2 = lrs[:, 1:, :, :, :].reshape(-1, c, h, w)

        offsets_backward,mask_backward = self.dfnet(lrs_1, lrs_2)
        offsets_backward = offsets_backward.view(n,t-1,2*self.offset_groups * self.kernel_size * self.kernel_size,h,w)
        mask_backward = mask_backward.view(n,t-1,self.offset_groups * self.kernel_size * self.kernel_size,h,w)
        # if self.is_mirror_extended:  # flows_forward = flows_backward.flip(1)
        #     flows_forward = None
        # else:
        #     flows_forward = self.spynet(lrs_2, lrs_1).view(n, t - 1, 2, h, w)
        offsets_forward,mask_forward = self.dfnet(lrs_2, lrs_1)
        offsets_forward = offsets_forward.view(n,t-1,2*self.offset_groups * self.kernel_size * self.kernel_size,h,w)
        mask_forward = mask_forward.view(n,t-1,self.offset_groups * self.kernel_size * self.kernel_size,h,w)
        return offsets_forward, offsets_backward,mask_forward,mask_backward
    ...
    def forward(self, lrs, to_cpu=False):
        ...
        for i in range(0, t):
            lr_curr = lrs[:, i, :, :, :]
            lr_curr_feat = outputs[i]
            if i > 0:  # no warping required for the first timestep # TODO 修改流的融合方式
                if flows_forward is not None:
                    flow = flows_forward[:, i - 1, :, :, :]
                    mask = mask_forward[:,i-1,:,:,:]
                else:
                    flow = flows_backward[:, -i, :, :, :]
                    mask = mask_forward[:,-i,:,:,:]
                # feat_prop = flow_warp(feat_prop, flow.permute(0, 2, 3, 1),padding_mode='border')
        ...
        # flows_forward, flows_backward = self.compute_flow(high_frequency_imgs) # TODO 修改流的计算
        flows_forward, flows_backward,mask_forward,mask_backward = self.compute_offset(high_frequency_imgs)
        final_out = self.FTT(
            bicubic_imgs, high_frequency_imgs,
            [flows_forward, flows_backward,mask_forward,mask_backward],
            [padding_h, padding_w], to_cpu) # TODO 修改流的应用
...
class FTT(nn.Module):
    ...
    def __init__(self, dct_kernel=(8,8), d_model=512, n_heads=8,flow_channel=2):
        ...
        input_channel = 192//3
        mid_channel = 512//2
        self.conv_layer1 = nn.Conv2d(input_channel, 512, 1, 1, 0, bias=True)
        ...
        self.conv_layer2 = nn.Conv2d(512, input_channel, 1, 1, 0, bias=True)
        self.hfi_warp = hfi_warp()
    def forward(self, bicubic_imgs, high_frequency_imgs, flows, padiings, to_cpu=False):
        ...
        if flows_forward is not None:
            ...
            flows_forward = flows_forward.view(n, t-1, self.flow_channel, h//self.dct_kernel[0], w//self.dct_kernel[1])
        flows_backward = resize_flow(flows_backward.view(-1, self.flow_channel, h, w), size_type='shape', sizes=(h//self.dct_kernel[0], w//self.dct_kernel[1]))
...
class DFNet(nn.Module):
    def __init__(self,pretrained=None,ou_ch=64,offset_groups=1,kernel_size=1):
        super().__init__()
        ...
        self.conv_offset = nn.Sequential(
            ...
            nn.Conv2d(self.out_channels, self.out_channels, 3, 1, 1),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
            # nn.Conv2d(self.out_channels, 27 * 16, 3, 1, 1),
            nn.Conv2d(ou_ch, 3 * offset_groups * kernel_size * kernel_size, 3, 1, 1),
        )
            # TODO 输出通道数
        if isinstance(pretrained, str):
            logger = get_root_logger()
    ...
    def forward(self,ref,supp):
        out = self.conv_offset(torch.cat([ref,supp],dim=1))
        o1,o2,mask = torch.chunk(out,3,dim=1)
        offset = 10 * torch.tanh(torch.cat((o1, o2), dim=1))# TODO 可改系数
        # offset1 ,offset2 = torch.chunk(out,2,dim=1)
...
def flow_warp(x,
    ...
    return output
class location_warp(nn.Module):
    def __init__(self):
        self.max_residue_magnitude = 10
        #super(offset_warp, self).__init__(out_ch,in_ch,groups,kernel_size)
        #self.init_offset()
        super().__init__()
        self.weight = nn.Parameter(torch.empty(2, 2 // 1, 3, 3))

        self.bias = nn.Parameter(torch.empty(2))
        self.stride=1
        self.padding=1
        self.dilation=1
        self.offset_conv = nn.Conv2d(72,18,3,1,1)
        self.mask_conv = nn.Conv2d(36,9,3,1,1)

    def forward(self, x, offsets, mask ):
        """
        x.shape [b,c,h,w] 1,2,60,60
        offset [b,2*offset_groups*kernel_h*kernel_w] 1*18*60*60
        mask [b,offset_groups*kernel_h*kernel_w]mask 1*9*60*60
        weight [b,]
        """
        ...
        if x.shape[1]>2:
            x_0 = x[:,:-2,:,:]
            x_1 = x[:,-2:,:,:]
            x_out = torchvision.ops.deform_conv2d(x_1, self.offset_conv(offsets), self.weight, self.bias, self.stride, self.padding,self.dilation,self.mask_conv(mask))
            return torch.cat([x_0,x_out],dim=1)
        else:
            return torchvision.ops.deform_conv2d(x, self.offset_conv(offsets), self.weight, self.bias, self.stride, self.padding,self.dilation,self.mask_conv(mask))
class hfi_warp(nn.Module):
    def __init__(self):
        self.max_residue_magnitude = 10
        #super(offset_warp, self).__init__(out_ch,in_ch,groups,kernel_size)
        #self.init_offset()
        super().__init__()
        self.weight = nn.Parameter(torch.empty(512, 512 // 1, 3, 3))

        self.bias = nn.Parameter(torch.empty(512))
        self.stride=1
        self.padding=1
        self.dilation=1
        self.offset_conv = nn.Conv2d(72,18,3,1,1)
        self.mask_conv = nn.Conv2d(36,9,3,8,1)
        # TODO 增大offset_groups
    def forward(self, x, offsets, mask ):
        """
        x.shape [b,c,h,w] 1,2,60,60
        offset [b,2*offset_groups*kernel_h*kernel_w] 1*18*60*60
        ...
        if False:
            ...
            return torchvision.ops.deform_conv2d(x, self.offset_conv(offsets), self.weight, self.bias, self.stride, self.padding,self.dilation,self.mask_conv(mask))
class offset_warp(nn.Module):
    def __init__(self, out_ch,in_ch,groups,kernel_size):
        self.max_residue_magnitude = 10
        #super(offset_warp, self).__init__(out_ch,in_ch,groups,kernel_size)
        #self.init_offset()
        super().__init__()
        self.weight = nn.Parameter(torch.empty(out_ch, in_ch // groups, kernel_size, kernel_size))
    ...
    def forward(self, x, offsets, mask ):
        """
        x.shape [b,c,h,w] 1,64,240,240
        offset [b,2*offset_groups*kernel_h*kernel_w]
        mask [b,offset_groups*kernel_h*kernel_w]
        weight [b,]
        """
        #offset = torch.cat([offset_1, offset_2], dim=1)

        return torchvision.ops.deform_conv2d(x, offsets, self.weight, self.bias, self.stride, self.padding,self.dilation,mask)
