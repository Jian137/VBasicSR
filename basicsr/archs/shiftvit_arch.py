...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=GroupNorm):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
...
class PatchUnEmbed(nn.Module):
    ...
    def forward(self, x, x_size):
        x = x.view(x.shape[0], self.embed_dim, x_size[0], x_size[1])  # b Ph*Pw c
        return x
...
if __name__ == '__main__':
    ...
    model = SwinIR(
        ...
        depths=[10, 18, 36, 10],
        embed_dim=128,
        num_heads=[10, 18, 36, 10],
        mlp_ratio=2,
        upsampler='pixelshuffledirect')
    print(model)
    x = torch.randn((1, 3, height, width))
    p,f = profile(model,inputs=(x,))
