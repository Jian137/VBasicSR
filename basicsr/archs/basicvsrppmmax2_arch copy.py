...
class BasicVSRPlusPlusMMAX2(nn.Module):
    ...
    def __init__(self,
        ...
        # optical flow
        self.spynet = SpyNet(spynet_path)# TODO 改掉SptNet
    ...
    def propagate(self, feats, flows, module_name):
        ...
        n, t, _, h, w = flows.size()# 删1
        n, t, c, h, w = feats['spatial'].size()
        ...
        feat_prop = flows.new_zeros(n, self.mid_channels, h, w)# 删除



        for i, idx in enumerate(frame_idx):
            # 遍历每一帧
            feat_current = feats['spatial'][mapping_idx[idx]] # 获取当前帧特征
            if self.cpu_cache:
                feat_current = feat_current.cuda()
                feat_prop = feat_prop.cuda() # 删除
            # second-order deformable alignment

            # 执行对齐模块
            if i > 0 and self.is_with_alignment:
                flow_n1 = flows[:, flow_idx[i], :, :, :] # 获取当前流 删除
                if self.cpu_cache:
                    flow_n1 = flow_n1.cuda()

                cond_n1 = flow_warp(feat_prop, flow_n1.permute(0, 2, 3, 1))# 删除

                # 预对齐 warped feature 部分

                # initialize second-order features
                feat_n2 = torch.zeros_like(feat_prop)
                flow_n2 = torch.zeros_like(flow_n1)# 删除
                cond_n2 = torch.zeros_like(cond_n1)# 删除
                ...
                feat_prop = self.deform_align[module_name](feat_prop, cond, flow_n1, flow_n2)#TODO 可变形卷积对齐 二阶对齐
                # cond:
                # 可变形模块结束
    ...
    def forward(self, lqs):
        ...
        flows_forward, flows_backward = self.compute_flow(lqs_downsample)# 删除光流部分
        # 计算光流 删除或者替换 self.compute_flow 采用查询模块替换  1
        # feature propgation
...
import torch
if __name__=="__main__":
    basicvsrpp = BasicVSRPlusPlusMMAX2()
    input = torch.rand(1,7,1,256,256)
    basicvsrpp(input)
