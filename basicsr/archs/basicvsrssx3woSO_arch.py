...
class BasicVSRSubSubX3woSecondOrder(nn.Module):
    # 无光流模块的basicvsr++
    ...
    def __init__(self,
        ...
        for i, module in enumerate(modules):
            ...
            self.backbone[module] = ConvResidualBlocks((1) * mid_channels, mid_channels, num_blocks)

        # upsampling module
        self.reconstruction = ConvResidualBlocks(5 * mid_channels, mid_channels, 5)

        #self.upconv1 = nn.Conv2d(mid_channels, mid_channels * 4, 3, 1, 1, bias=True)
        self.upconv2 = nn.Conv2d(mid_channels, 64 * 9, 3, 1, 1, bias=True)

        self.pixel_shuffle = nn.PixelShuffle(3)

        self.conv_hr = nn.Conv2d(64, 64, 3, 1, 1)
        self.conv_last = nn.Conv2d(64, 1, 3, 1, 1)#TODO npy输出为1通道
        self.img_upsample = nn.Upsample(scale_factor=3, mode='bilinear', align_corners=False)
    ...
    def propagate(self, feats, module_name):
        ...
        for i, idx in enumerate(frame_idx):
            ...
            if i > 0 and self.is_with_alignment:
                ...
                if False:  # second-order features
            ...
            feat_prop = feat_prop + self.backbone[module_name](feat_prop)# 修改
