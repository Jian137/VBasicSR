...
from basicsr.utils import FileClient, bgr2ycbcr, npyfrompath, img2tensor
...
class PairedNpyDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            # flip, rotation
            #TODO 数据扩增修改 img_gt, img_lq = augment([img_gt, img_lq], self.opt['use_hflip'], self.opt['use_rot'])
        ...
        # normalize
        img_gt = torch.from_numpy(img_gt).unsqueeze(0)
        img_lq = torch.from_numpy(img_lq).unsqueeze(0)
